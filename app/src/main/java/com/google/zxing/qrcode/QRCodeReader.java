...
public class QRCodeReader implements Reader {
  ...
  public final Result decode(BinaryBitmap image, Map<DecodeHintType,?> hints)
    ...
    if (hints != null && hints.containsKey(DecodeHintType.PURE_BARCODE)) {
      ...
      if (scaleCameraZoom(detectorResult)){
       return  null;
      }
  ...
  private boolean  scaleCameraZoom(DetectorResult detectorResult ){
    boolean isSCale=false;
    if(CameraManager.get().getCaptureActivity()!=null){
...
//      主要获得两个参数： frameRect，Camera对象
      CameraManager cameraManager = CameraManager.get();
      Rect frameRect = cameraManager.getFramingRect(false);
      Log.e("QRCodeReader", "scaleCameraZoom ;len = "+len+";resultPoints.toString  = "+ Arrays.toString(p)+";frameRect = "+frameRect);
      if(frameRect!=null){
        ...
        if(parameters.isZoomSupported()){
          if(len <= frameWidth/4) {//二维码在扫描框中的宽度小于扫描框的1/4，放大镜头
            ...
            Log.e("QRCodeReader", "scaleCameraZoom 开始放大摄像头了  ");
            return true;
          }
        }
      }
